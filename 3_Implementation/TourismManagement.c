...
user* InitializeList(user *h)
{
    ...
    rewind(fp);
	while(fscanf(fp,"%s %s %s %f %d",temp.uname,temp.pword,temp.pl,&temp.rate,&temp.nti)!=EOF)
	{
		ptr=(user*)malloc(sizeof(user));
		strcpy(ptr->uname,temp.uname);
		strcpy(ptr->pword,temp.pword);
		strcpy(ptr->pl,temp.pl);
		ptr->rate=temp.rate;
		ptr->nti=temp.nti;
		ptr->next=NULL;
...
void WriteToFile(user *h)
{
    ...
    while(h!=NULL)
    {
        fprintf(fp,"%s %s %s %f %d\n",h->uname,h->pword,h->pl,h->rate,h->nti);
        h=h->next;
    }
    fclose(fp);
}
...
void CheckTicket(user *h)
{
    while(h!=NULL)
    {
        if(!strcmp(h->uname,currentuser))
            break;
        h=h->next;
    }
    if(!strcmp(h->pl,"\0") || h->rate==0.0 || h->nti==0)
    {
...
user* AddUser(user* h)
{
    ...
    fflush(stdin);
    printf("Enter uname or email\n");
    scanf("%s",nw->uname);
    while(h!=NULL)
    {
        if(!strcmp(h->uname,nw->uname))
        {
...
void LoginUser(user* h)
{
    ...
    while(h!=NULL)
    {
        if((!strcmp(h->uname,uname)) && (!strcmp(h->pword,pword)))
        {
        ...
        else if((!strcmp(h->uname,uname)) && (strcmp(h->pword,pword)))
        {
            printf("pword mismatch\n");
            return;
        }
        h=h->next;
    }
...
void BookTicket(user *h)
{
    ...
    ShowBrochure();
    float ratelist[]={40000.0,60000.0,25000.0,38000.0,120000.0,10000.0,30000.0,32000.0,45000.0,250000.0};
    fflush(stdin);
    printf("\nEnter pl code (eg: LL, JK)\n");
    scanf(" %[^\n]s",pl);
    char choice;
    fflush(stdin);
    ...
    if(strcmp(pl,"LL")==0)
        rate=ratelist[0];
    else if(strcmp(pl,"JK")==0)
        rate=ratelist[1];
    else if(strcmp(pl,"SK")==0)
        rate=ratelist[2];
    else if(strcmp(pl,"SHM")==0)
        rate=ratelist[3];
    else if(strcmp(pl,"AND")==0)
        rate=ratelist[4];
    else if(strcmp(pl,"BHB")==0)
        rate=ratelist[5];
    else if(strcmp(pl,"AG")==0)
        rate=ratelist[6];
    else if(strcmp(pl,"ND")==0)
        rate=ratelist[7];
    else if(strcmp(pl,"RJ")==0)
        rate=ratelist[8];
    else if(strcmp(pl,"SI")==0)
        rate=ratelist[9];
    else
    {
...
void PrintTicket(user *h)
{
    ...
    fprintf(fp,"Email ID: %s\nTour Code: %s\nTicket Cost: Rs %f\nNumber of tickets: %d\nTotal Cost: Rs %f\n",h->uname,h->pl,h->rate,h->nti,total);
    fclose(fp);
}
//TMS1353f
void CancelTicket(user *h)
{
    ...
    if(strcmp(h->pl,"LL")==0)
        flag++;
    else if(strcmp(h->pl,"JK")==0)
        flag++;
    else if(strcmp(h->pl,"SK")==0)
        flag++;
    else if(strcmp(h->pl,"SHM")==0)
        flag++;
    else if(strcmp(h->pl,"AND")==0)
        flag++;
    else if(strcmp(h->pl,"BHB")==0)
        flag++;
    else if(strcmp(h->pl,"AG")==0)
        flag++;
    else if(strcmp(h->pl,"ND")==0)
        flag++;
    else if(strcmp(h->pl,"RJ")==0)
        flag++;
    else if(strcmp(h->pl,"SI")==0)
        flag++;
    else
    {
...
void Changepword(user *h)
{
    user *t=h;
    char passcurr[100];
    fflush(stdin);
    printf("Enter your current pword to continue:\n");
    scanf(" %[^\n]s",passcurr);
    while(h!=NULL)
    {
        if(!strcmp(h->uname,currentuser))
    ...
    if(h==NULL)
        return;
    if(!strcmp(passcurr,h->pword))
    {
        printf("Enter new pword:\n");
        scanf(" %[^\n]s",h->pword);
    }
    WriteToFile(t);
}
